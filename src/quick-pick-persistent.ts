...
import { PersistentState, type PersistentStorage } from "./persistent-state";
import type { BaseQuickPickItem, QuickPickValue } from "./quick-pick";
import {
  GenericQuickPick,
  QuickPickButtons,
  QuickPickSeparator,
...
/**
 * Specialization of {@link QuickPickWrapper} which persists selected items and preselects them on
 * next invokation.
 *
 ...
 */
export class PersistentQuickPickWrapper<T, Q extends BaseQuickPickItem> extends QuickPickWrapper<
  T,
  Q
> {
  ...
  override async select(): Promise<T | undefined> {
    ...
    this.items = lodash.sortBy(mruItems, (x) => x.index).map((item) => item.item);

    const selected = await super.select();
    if (selected) {
      const label = this.toPersistentLabel(selected);
      arrayRemove(labels, label);
  ...
  override async selectMany(): Promise<readonly T[] | undefined> {
    ...
    this.selectedItems = QuickPickWrapper.filterOutSeparators(this.items).filter((item) =>
      labels.includes(this.toPersistentLabel(item)),
    );
...
/**
 * Works similarly to {@link GenericQuickPick} but uses persistent storage as described in
 * {@link PersistentQuickPickWrapper}.
 */
...
/**
 * QuickPick that allows to input value and shows MRU history of previously inputted values.
 */
export class PersistentInputHistoryQuickPick extends PersistentStringQuickPick {
  constructor(persistentKey: string) {
    ...
    this.addButton(QuickPickButtons.CLEAR_ALL, this.onDidTriggerClearAll.bind(this));

    this.addCommonItemButton(QuickPickButtons.REMOVE, this.onDidTriggerRemove.bind(this));
  }
  ...
  private async onDidTriggerClearAll() {
    const confirmed = await Message.askModal("warn", "Clear all items?", "No", "Yes");
    if (confirmed !== "Yes") return;
    arrayClear(this.origItems);
    this.onDidChangeValueImpl(this.qp.value);
  }

  private async onDidTriggerRemove(value: string) {
    const removed = arrayRemove(this.origItems, value);
    assert(removed);
    this.onDidChangeValueImpl(this.qp.value);
