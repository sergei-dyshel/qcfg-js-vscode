import { arrayFirstOf, arrayGroup } from "@sergei-dyshel/typescript/array";
import { assert, assertNotNull, notNull } from "@sergei-dyshel/typescript/error";
import type { QuickInputButton, QuickPick, QuickPickItem, QuickPickItemButtonEvent } from "vscode";
import { Disposable, type Location, QuickPickItemKind, ThemeIcon, window, workspace } from "vscode";
import { DisposableCollection } from "./disposable";
import { listen } from "./error-handling";
import { compareLocation } from "./position";
import { URI } from "./uri";
import { getActiveTextEditor, showTextDocument } from "./window";
...
export namespace QuickPickButtons {
   ...
   * https://code.visualstudio.com/api/references/icons-in-labels#icon-listing for full list.
   */
  export function create(themeIconId: string, tooltip?: string): QuickInputButton {
    return {
 ...
 *
 * Instead of requiring item type to extend {@link QuickPickItem} it can use any type by providing
 * converters.
 *
 * Unlike original `QuickPick` sets {@link QuickPick.sortByLabel} to `false` by default.
 *
 * NOTE: Manipulation with items uses boxing/unboxing and iteration so the implementation is not
 * suitable for QuickPicks with large number of items.
 */
export class QuickPickWrapper<T, Q extends BaseQuickPickItem> extends DisposableCollection {
  protected readonly qp: QuickPick<Q | QuickPickSeparator>;

  private readonly globalButtons = new Map<QuickInputButton, () => void | Promise<void>>();

  private readonly commonItemButtons = new Map<QuickInputButton, (_: T) => void | Promise<void>>();
  ...
  constructor(
    ...
    this.disposables.push(
      this.qp,
      listen(this.qp.onDidTriggerButton, this.onDidTriggerButtonImpl.bind(this)),
      listen(this.qp.onDidTriggerItemButton, this.onDidTriggerItemButtonImpl.bind(this)),
    );
  }
   ...
   */
  addCommonItemButton(button: QuickInputButton, cb: (_: T) => void | Promise<void>) {
    assert(this.qp.items.length === 0, "Adding item button after setting `items` - probably bug");
  ...
  set onDidActivateItem(cb: (value: T) => void | Promise<void>) {
    this.disposables.push(
      listen(this.qp.onDidChangeActive, (activeItems) => {
        if (activeItems.length === 0) return;
        assert(activeItems.length === 1, "multiple items are activated in QuickPick");
  ...
  set onDidChangeValue(cb: (value: string) => void | Promise<void>) {
    this.disposables.push(listen(this.qp.onDidChangeValue, cb));
  }
  ...
  set items(values: ReadonlyArray<T | QuickPickSeparator>) {
    this.qp.items = values.map((value) => {
      ...
      return {
        ...wrapped,
        buttons: [...(wrapped.itemButtons?.keys() ?? []), ...this.commonItemButtons.keys()],
      };
    });
  }
   ...
   *
   * Supports only setting. Use {@link QuickPickWrapper.onDidActivateItem} to track value;
   */
  set activeItem(value: T) {
    const active = arrayFirstOf(
      this.qp.items,
      (item) => !(item instanceof QuickPickSeparator) && this.unwrap(item) === value,
    );
  ...
  set selectedItems(values: readonly T[]) {
    ...
    this.qp.selectedItems = values.map((value) =>
      notNull(arrayFirstOf(items, (item) => this.unwrap(item) === value)),
    );
  }
  ...
  /**
   * Just show the QuickPick and run supplied callback when user accepts an item, without closing
   * the QuickPick.
   */
  async showOnly(onDidAcceptItem: (item: T) => void | Promise<void>): Promise<void> {
  ...
  protected static filterOutSeparators<T>(items: ReadonlyArray<T | QuickPickSeparator>) {
    return items.filter((item) => !(item instanceof QuickPickSeparator)) as readonly T[];
  }
  ...
  /**
   * @param onDidAcceptItem When given, run when user accepts item (presses ENTER or selects with
   *   mouse) without hiding QuickPick, only allow closing by pressing ESC
   */
  private async selectImpl(
    ...
    const accepted = await new Promise<boolean>((resolve) => {
      const disposer = Disposable.from(
        listen(this.qp.onDidAccept, async () => {
          if (this.qp.selectedItems.length === 0) return;
          if (onDidAcceptItem) return onDidAcceptItem(this.selectedItem!);
          didAccept = true;
          resolve(true);
          this.qp.hide();
        }),
        listen(this.qp.onDidHide, () => {
 ...
 */
export class StringQuickPick extends QuickPickWrapper<string, { label: string; detail?: string }> {
 ...
 */
export class GenericQuickPick<T> extends QuickPickWrapper<T, QuickPickValue<T>> {
  ...
  constructor(
    ...
    /**
     * Sometimes its convenient to pass list of items as parameter to allow vscode auto-infer
     * QuickPick type
     */
    items?: ReadonlyArray<T | QuickPickSeparator>,
  ) {
    super(GenericQuickPick.createWrap(toQuickPickItem), GenericQuickPick.defaultUnwrap, items);
  }
}
 ...
 */
export class QuickPickLocations<T> extends QuickPickWrapper<T, QuickPickLocation<T>> {
  ...
  constructor(
    ...
    this.disposables.push(
      listen(this.qp.onDidChangeActive, QuickPickLocations.onDidChanceActiveImpl),
    );
  }
  ...
  setAndGroupItems(items: readonly T[]) {
    ...
    rawItems.sort((a, b) => compareLocation(a.location, b.location));

    const groups = arrayGroup(rawItems, (a, b) => URI.equals(a.location.uri, b.location.uri));
  ...
  /**
   * Choose active item to be near current editor location. If no editor active does nothing.
   */
  adjustActiveItem() {
    ...
    for (const item of items) {
      ...
      if (
        URI.equals(loc.uri, editor.document.uri) &&
  ...
  private static async onDidChanceActiveImpl<T>(activeItems: readonly QuickPickItem[]) {
    if (activeItems.length === 0) return;
