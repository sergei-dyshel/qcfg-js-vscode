...
declare module "vscode" {
  ...
  export interface RemoteAuthorityResolverContext {
    ...
    /**
     * Exec server from a recursively-resolved remote authority. If the remote authority includes
     * nested authorities delimited by `@`, it is resolved from outer to inner authorities with
     * ExecServer passed down to each resolver in the chain.
     */
  ...
  export interface ResolvedOptions {
    ...
    /**
     * When provided, remote server will be initialized with the extensions synced using the given
     * user account.
     */
  ...
  /**
   * Used as part of the ResolverResult if the extension has any candidate, published, or forwarded
   * ports.
   */
  export interface TunnelInformation {
    /**
     * Tunnels that are detected by the extension. The remotePort is used for display purposes. The
     * localAddress should be the complete local address (ex. localhost:1234) for connecting to the
     * port. Tunnels provided through detected are read-only from the forwarded ports UI.
     */
  ...
  /**
   * An ExecServer allows spawning processes on a remote machine. An ExecServer is provided by
   * resolvers. It can be acquired by `workspace.getRemoteExecServer` or from the context when in a
   * resolver (`RemoteAuthorityResolverContext.execServer`).
   */
  export interface ExecServer {
     ...
     * @returns A promise that gives access to the process' stdin, stdout and stderr streams, as
     *   well as the process' exit code.
     */
    spawn(
      command: string,
      args: string[],
    ...
    /**
     * Spawns an connector that allows to start a remote server. It is assumed the command starts a
     * Code CLI. Additional arguments will be passed to the connector.
     *
     * @param command The command to execute. It is assumed the command spawns a Code CLI
     *   executable.
     * @param args The arguments to pass to the connector
     * @param options Additional options for the spawned process.
     * @returns A promise that gives access to the spawned {@link RemoteServerConnector}. It also
     *   provides a stream to which standard log messages are written.
     */
    ...
    /**
     * Downloads the CLI executable of the desired platform and quality and pipes it to the provided
     * process' stdin.
     *
     * @param buildTarget The CLI build target to download.
     * @param command The command to execute. The downloaded bits will be piped to the command's
     *   stdin.
     * @param args The arguments to pass to the command.
     * @param options Additional options for the spawned process.
     ...
     * @returns A duplex stream, and a promise the resolves when both sides have closed.
     */
    tcpConnect(
      host: string,
      port: number,
  ...
  export interface RemoteServerConnector {
     ...
     * @returns A promise that resolves to a {@link ManagedMessagePassing} object that can be used
     *   with a resolver
     */
  ...
  export interface CliBuild {
    readonly quality: string;
    /** 'LinuxAlpineX64' | 'LinuxAlpineARM64', 'LinuxX64' | 'LinuxARM64' | 'LinuxARM32' | 'DarwinX64'
| 'DarwinARM64' | 'WindowsX64' | 'WindowsX86' | 'WindowsARM64' */
  ...
  export interface RemoteFileSystem {
     ...
     * @throws If an i/o error happens during removal. It does not throw if the path already does
     *   not exist.
     */
     ...
     * @returns A writable `stream` that accepts data, and a `done` promise that will resolve after
     *   `stream.end()` is called once the write is complete.
  ...
  export interface RemoteAuthorityResolver {
     ...
     *
     * This method will be invoked once during the startup of the editor and again each time the
     * editor detects a disconnection.
     *
    ...
    /**
     * Resolves an exec server interface for the authority. Called if an authority is a midpoint in
     * a transit to the desired remote.
     *
     * @param authority The authority part of the current opened `vscode-remote://` URI.
    ...
    /**
     * Can be optionally implemented if the extension can forward ports better than the core. When
     * not implemented, the core will use its default forwarding logic. When implemented, the core
     * will use this to forward ports.
     *
     * To enable the "Change Local Port" action on forwarded ports, make sure to set the
     * `localAddress` of the returned `Tunnel` to a `{ port: number, host: string; }` and not a
     * string.
     */
    tunnelFactory?: (
    ...
    /**
     * P Provides filtering for candidate ports.
     */
  ...
  export namespace env {
    /**
     * The authority part of the current opened `vscode-remote://` URI. Defined by extensions, e.g.
     * `ssh-remote+${host}` for remotes using a secure shell.
     *
     * Note* that the value is `undefined` when there is no remote extension host but that the value
     * is defined in all extension hosts (local and remote) in case a remote extension host exists.
     * Use {@link Extension.extensionKind} to know if a specific extension runs remote or not.
     */
